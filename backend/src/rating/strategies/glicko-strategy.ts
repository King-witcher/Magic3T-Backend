...
export class GlickoStrategy extends RatingStrategy {
  ...
  async update(a: UserRow, b: UserRow, aScore: number) {
    ...
    b.glicko = {
      rating: newRating(b.glicko, a.glicko, 1 - aScore, inflation, config.max_rd),
      deviation: Math.max(
        config.min_rd,
  ...
  async getTotalLp(user: UserRow): Promise<number> {
    ...
    const rawLP = Math.round(
      400 * ((user.glicko.rating - config.base_score) / config.league_length + config.base_league)
    )
    return rawLP
  }
  ...
  async getRatingProgress(user: UserRow): Promise<number> {
    ...
    const currentRD = await this.getCurrentRD(user.glicko.deviation, user.glicko.timestamp)

    if (currentRD <= config.rd_threshold) return 100
    return ((config.max_rd - currentRD) / (config.max_rd - config.rd_threshold)) * 100
  }

  private getC(inflationTimeInDays: number): number {
    const ONE_DAY = 24 * 60 * 60 * 1000 // milliseconds in one day
    return Math.sqrt((DEFAULT_MAX_RD ** 2 - DEFAULT_MIN_RD ** 2) / (inflationTimeInDays * ONE_DAY))
  }
