...
export class UserService {
  ...
  async changeNickName(userId: string, newNickname: string) {
    ...
    if (
      user.identification &&
      Date.now() - user.identification.last_changed.getTime() < 1000 * 60 * 60 * 24 * 30
    ) {
      throw new BaseError(
    ...
    if (user.identification?.nickname === newNickname) {
      throw new BaseError('nickname must be different from the current', HttpStatus.BAD_REQUEST)
    }

    // Nickname unavailable
    const nicknameOwner = await this.userRepository.getByNickname(newNickname)
    if (nicknameOwner) {
      throw new BaseError('the nickname is already being used', HttpStatus.BAD_REQUEST)
    }
  ...
  async getRanking(): Promise<UserPayload[]> {
    const [bestPlayers, _ratingConfig] = await Promise.all([
  ...
  async register(userId: string, body: RegisterUserCommand) {
    const user = await this.userRepository.get(userId)
    if (user) throw new BaseError('user already registered', HttpStatus.BAD_REQUEST)
