    ...
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="text-center mt-4 space-y-2">
        ...
        {/* <div className="flex items-center justify-center gap-3">
          <img src={leagueInfo.icon} alt={leagueInfo.name} className="size-6" />
          <span className="text-lg text-gold-2 font-semibold">
            {leagueInfo.name} {!isApex && divisionString}
          </span>
          {user.rating.points !== null && (
            <span className="text-grey-1">{user.rating.points} LP</span>
          )}
        </div> */}
      </div>
    </div>
